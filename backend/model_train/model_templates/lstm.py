...
def build_lstm_model(input_shape):
    ...
    model.add(Dense(1))  # Output layer for RUL regression
...
def train_lstm(X_train, y_train, X_test, y_test):
    ...
    architecture = {
                    "model": {
                        "type": "Sequential",
                        "layers": [
                        {"LSTM": {"units": 64, "return_sequences": "true", "time_steps": 10}},
                        {"Dropout": {"rate": 0.2}},
...
def main():
    ...
    try :
        ...
        # Fit the model
        model , architecture = train_lstm(X_train, y_train, X_test, y_test)
        print("Model trained.")
